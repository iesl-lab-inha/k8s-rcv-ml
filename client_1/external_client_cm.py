...
def migration_request(ml_type, client_id, runtime):
    try:
        tm = str(datetime.utcnow().isoformat(sep=' ', timespec='milliseconds'))
        para_dict = {"type" : ml_type, "clientID" : client_id, "clienttime" :tm, "time" : runtime}
        print(str(para_dict))
        resp1 = requests.get(EDGE, params=para_dict)
        return resp1;
    except Exception as e:
        print(e)
        ...
        if data == 'Model ready':
            ...
            while True:
                try:
                    ...
                    if (int(elapsed_time - start_time) >= 180):
                        print("moving away from the nearest edge server")
...
async def image_connect(ws, port):
    async with websockets.connect((ws+":"+port)) as websocket:
        ...
        if os.path.exists('/nvme/sri/client/Carla1/') is True: 
            #images load Loop
            datalist = os.listdir('/nvme/sri/client/Carla1/')
            start_time = time.time()
            res_all = 0
            runt_time = 0
            cnt = 0
            ...
            for item in datalist:
                ...
                item_path = os.path.join('/nvme/sri/client/Carla1/',item)
                ...
                if (int(elapsed_time - start_time) >= 180):
                    print("moving away from the nearest edge cluster")
                    print("Result time : {}, Bandwidth : {}, all request : {}".format(res_all/cnt, bnw/cnt, cnt));
                    #break
                    run_time = 180;
                    print(int(run_time))
    ...
    try:
        ...
        if resp.json()['migration'] is not 1:
            ...
            if sys.argv[1] == 'scalar':
                time.sleep(5)
                runtime = asyncio.get_event_loop().run_until_complete(scalar_connect(EDGE_WS, num_port))
                print("run_time : {}".format(runtime))
                resp1 = migration_request(sys.argv[1], sys.argv[2], runtime)
                if resp1.json()['migration1'] is not 1:
                    print("ok done")
                    port = resp1.json()['service_port']
                    print("port = {}".format(port))
                    time.sleep(5)
                    asyncio.get_event_loop().run_until_complete(scalar_connect(EDGE_WS, port))
            elif sys.argv[1] == 'image':
                time.sleep(16)
                runtime = asyncio.get_event_loop().run_until_complete(image_connect(EDGE_WS, num_port))
                print("run_time : {}".format(runtime))
                resp1 = migration_request(sys.argv[1], sys.argv[2], runtime)
                if resp1.json()['migration1'] is not 1:
                    print("ok done")
