...
def create_deployment_object(ml_type):
    ...
    if ml_type == 'image':
        res = client.V1ResourceRequirements(requests={"memory":"4394531Ki"},limits={"memory":"4882812Ki"})
    ...
    #Container obj
    container = client.V1Container(name = ml_type, image=('wnghks3030/'+ ml_type +':final'), ports=[client.V1ContainerPort(container_port=5700)],resources=res)
    #Pod obj
...
def service_request(): 
    ...
    service_port = {'scalar':'32220', 'image':'30030'} #scalar-w, image-w external port num
    ...
    if deploy_exist == 1:
        if ml_type == 'scalar':
            ...
            image_dp_resp = app_v1.patch_namespaced_deployment_scale(name=(ml_type+'-deploy'), namespace=nsp, body=image_deploy)
    
    #Step5. new replicas wait
    dps = app_v1.read_namespaced_deployment(name = nm, namespace=nsp)
    wait_time = 0;
    while scalar_deploy.spec.replicas != dps.status.available_replicas:
        wait_time = wait_time + 1;
        sleep(1);
        print("Wait for {}sec ready for replicas".format(wait_time));
        dps = app_v1.read_namespaced_deployment(name = nm, namespace = nsp)
    print("Wait for 10sec ready for model");
