   ...
   for item in list(dps.items):
       if item.metadata.name == nm:
           if ml_type == 'scalar':
               ...
               scalar_dep_resp = app_v1.patch_namesapced_deployment_scale(name=nm, namespace=nsp, body = scalar_deploy)
           elif ml_type == 'image':
               image_deploy = create_deployment_object('image')
               iamge_deploy.spec.replicas = item.spec.replicas - 1
...
def service_request(): 
    ...
    service_port = {'scalar':'32220', 'image':'30030'}

    #read nodes list, node's count
    #xavier_label = "machine=xavier"
    #tx2_label = "machine=tx2"
    xavier_nodelist_rsp = core_v1.list_node(label_selector="machine=xavier")
    tx2_nodelist_rsp = core_v1.list_node(label_selector="machine=tx2")
