...
EDGE2 = 'http://165.246.41.45:32759/app-service'
#EDGE2 = 'http://192.168.1.14:5620/app-service'

EDGE_WS = "ws://165.246.41.45"

worker = 'http://192.168.1.21:5633/app-service'

def request_to_edge2(ml_type, client_id):
    try:
        tm = str(datetime.utcnow().isoformat(sep=' ', timespec='milliseconds'))
        para_dict={"type" : ml_type, "clientID" : client_id, "clienttime" : tm, "time" : 0}
        print(str(para_dict))
...
def request_to_worker(ml_type, runtime):
    try:
        para_dict={"type" : ml_type, "runtime" : runtime}
        print(str(para_dict))
        worker_response = requests.get(worker, params=para_dict)
...
def container_to_edge2(ml_type, client_id, runtime):
    try:
        tm = str(datetime.utcnow().isoformat(sep=' ', timespec='milliseconds'))
        para_dict = {"type" : ml_type, "clientID" : client_id, "clienttime" :tm, "time" : runtime}
        print(str(para_dict))
        container_migration = requests.get(EDGE2, params=para_dict)
        return container_migration;
    except Exception as e:
        print(e)
...
def service_request(): 
    ...
    runtime = request.args.get('time',type = str)
    
    print('\nRequest message')
    print(ml_type+'\t'+client_id+'\t'+tm+'\t'+runtime+'\t')
    print("\nCurrent time = " + datetime.utcnow().isoformat(sep=' ',timespec='milliseconds'))
    nsp = (ml_type+'-test')
    nm = (ml_type+'-deploy')

    if runtime == '180':
        print("runtime is 180")
        #print("Listing pod with their container:")
        #ret = core_v1.list_namespaced_pod(namespace = "scalar-test")
         
        #print(ret.items[0].status.pod_ip)
        #print(ret.items[0].metadata.name)
        #print(ret.items[0].status.container_statuses)
        
        worker_response = request_to_worker(ml_type, runtime)
        print(worker_response)
        if worker_response.json()['push'] is 1:
            print("push completed")
            container_migration = container_to_edge2(ml_type, client_id, runtime)
            print(container_migration)
            if container_migration.json()['migration1'] is not 1:
                print("container migration to edge 2")
                num_port = container_migration.json()['service_port']
        return jsonify({'service_port': num_port, 'migration1':0}) 
    elif runtime == '0':
    ...
    if deploy_exist == 1:
        if Remaining_memory < int(resource_limit):
            ...
            if resp2.json()['migration1'] is not 1:
                ...
                num_port = resp2.json()['service_port']
                print("\nservice port for edge 2 : {}".format(num_port))
            elif resp2.json()['migration1'] is 1:
                print('Offloading')
            return jsonify({'service_port': num_port, 'migration':1})        
        else:
