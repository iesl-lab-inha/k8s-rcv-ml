...
def service_request(): 
    ...
    service_port = {'scalar':'31604', 'image':'xxxxx'}
    ...
    ml_capacity = xavier_count*3 + tx2_count*3;
    ...
    if deploy_exist == 1:
        if scalar_deploy.spec.replicas > ml_capacity:
            ...
            return jsonify({'service_port': service_port[ml_type], 'migration2':1})
    ...
    return jsonify({'service_port' : service_port[ml_type],'migration2' : 0})

#####main#####
if __name__ == '__main__':
    app.debug = True
    app.run(host = '192.168.1.11', port=5613)
