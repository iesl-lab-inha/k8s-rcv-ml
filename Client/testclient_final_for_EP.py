...
#EDGE = 'http://192.168.1.20:5601/app-service' 
CENTER = 'http://ec2-3-34-2-200.ap-northeast-2.compute.amazonaws.com'
...
if __name__ == "__main__":
    try:
        ...
        if resp.json()['offloading'] is not 1:
            ...
            if sys.argv[1] == 'scalar':
                time.sleep(4)
                asyncio.get_event_loop().run_until_complete(CENTER, scalar_connect('32222')) #nodeport
            elif sys.argv[1] == 'image':
                time.sleep(16)
                asyncio.get_event_loop().run_until_complete(CENTER, image_connect('30033')) #nodeport
            thread_api.join()
    except Exception as e:
        print(e)
