...
async def object_connect(ws, port):
    async with websockets.connect((ws+":"+port)) as websocket:
    ...
    try:
        ...
        if resp.json()['offloading'] is not 1:
            ...
            if sys.argv[1] == 'scalar':
                ...
                #time.sleep(xx) ************************check time sleep
                asyncio.get_event_loop().run_until_complete(object_connect(EDGE_WS, num_port))
            ...
            if sys.argv[1] == 'scalar':
                ...
                time.sleep(16)
                asyncio.get_event_loop().run_until_complete(CENTER, image_connect('5700')) #nodeport
            elif sys.argv[1] == 'object':
                #time.sleep(xx) ************************check time sleep
                asyncio.get_event_loop().run_until_complete(CENTER, object_connect('5700'))
            thread_api.join()
    except Exception as e:
        print(e)
