...
CENTER = 'http://ec2-3-135-204-38.us-east-2.compute.amazonaws.com'
#CENTER_WS = 'ws://172.31.7.139'
CENTER_WS = 'ws://3.135.204.38'
...
async def image_connect(ws, port):
    async with websockets.connect((ws+":"+port)) as websocket:
        ...
        if os.path.exists('../ep0') is False:
            print('no ../ep0 directory')
...
if __name__ == "__main__":
    try:
        ...
        if sys.argv[1] == 'scalar':
            ...
            time.sleep(16)
            asyncio.get_event_loop().run_until_complete(image_connect(CENTER_WS, '5700')) #nodeport
        thread_api.join()
    except Exception as e:
        print(e)
