...
##CENTER = 'http://ec2-18-119-141-27.us-east-2.compute.amazonaws.com'
CENTER = 'http://172.31.7.139'
...
if __name__ == "__main__":
    try:
        ...
        if sys.argv[1] == 'scalar':
            time.sleep(4)
            asyncio.get_event_loop().run_until_complete(scalar_connect(CENTER, '/driver-class-predict')) #nodeport
        elif sys.argv[1] == 'image':
            time.sleep(16)
            asyncio.get_event_loop().run_until_complete(image_connect(CENTER, '8080')) #nodeport
        thread_api.join()
    except Exception as e:
        print(e)
