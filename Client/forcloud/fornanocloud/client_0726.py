...
CENTER = 'http://165.246.41.238'
#CENTER_WS = 'ws://172.31.7.139'
CENTER_WS = 'ws://165.246.41.238'
...
def request_to_center(ml_type, client_id):
    ...
    if ml_type == 'scalar':
        resp = requests.post((CENTER+'/driver-class-predict'))
    #elif ml_type == 'image':
...
async def scalar_connect(ws, port):
    ...
    #async with websockets.connect(ws+port) as websocket:
        ...
        for skiprow in range(0,40):
            ...
            #print('before send, stada1: '+stdata1)
            await websocket.send(stdata1)
            #print('after send stada1')
            ...
            while True:
                ...
                try:
                    ...
                    for i in range(0,40):
                        ...
                        #simpledata = '{"Long_Term_Fuel_Trim_Bank1": 0.76634857}'
                        #print('before send') 
                        await websocket.send(stdata) #간단한 데이터 보내보기
...
if __name__ == "__main__":
    try:
        ...
        if resp.json()['offloading'] is not 1:
            ...
            #elif sys.argv[1] == 'image':
            #    time.sleep(16)
            #    asyncio.get_event_loop().run_until_complete(image_connect(CENTER_WS, '31102')) #nodeport
            #thread_api.join()
    except Exception as e:
        print(e)
